...
void __fastcall TmainForm::BaseLoop(TObject *, int k)
{
	...
	long double dTptime = -gammatep*(T_in[k])/dtime; //попытка посчитать температурные деформации
	T_old[k]+=T_in[k];
	T_in[k]=0.0;

	// КОМПОНЕНТЫ ТЕНЗОРА СКОРОСТЕЙ ДЕФОРМАЦИЙ
	long double epsdotrr = 	((z2 - z3) * u1 + (z3 - z1) * u2 + (z1 - z2) * u3 ) / (as[k] + aks[k]) + dTptime;//;
	long double epsdotrz =	((r3 - r2) * u1 + (r1 - r3) * u2 + (r2 - r1) * u3 + (z2 - z3) * v1 +
							(z3 - z1) * v2 + (z1 - z2) * v3) / (2. * (as[k] + aks[k])) + dTptime;//;
	...
	{
		Heps =	2. * Gs[k] + ks[k] * powl(k1s[k], 2) - k1s[k] * k1s[k] / ks[k] - alphas[k] / F[k] *
				(srr[k] * (epsrrp[k] - epsttp[k]) + szz[k] * (epszzp[k] - epsttp[k]) + 
				2. * srz[k] * epsrzp[k]);
	}
	...
	psi[k] =	sqrtl(2. * (powl(srr[k], 2.) + powl(szz[k], 2.) + 
				powl(srz[k], 2.) + srr[k] * szz[k])) - F[k];

	// ПРОИЗВОДНАЯ ФУНКЦИИ ДЕФОРМИРОВАНИЯ ПО ВРЕМЕНИ
	// (2015 год) За такое надо руки отрывать!
	psidot[k] =	2.e0 * Gs[k] / F[k] * (srr[k] * (epsdotrr - epsdottt) +
				szz[k] * (epsdotzz - epsdottt) + 2.e0 * srz[k] * epsdotrz) -
	...
	long double epsdotrrp =
		H * psidot[k] / Heps * (srr[k] / F[k] - alf * (3. * ks[k] - k1s[k] * Gs[k]) /
		(3. * Gs[k]));
	long double epsdotzzp =
		H * psidot[k] / Heps * (szz[k] / F[k] - alf * (3. * ks[k] - k1s[k] * Gs[k]) /
		(3. * Gs[k]));
	long double epsdotttp =
		H * psidot[k] / Heps * (stt[k] / F[k] - alf * (3. * ks[k] - k1s[k] * Gs[k]) /
		(3. * Gs[k]));
	...
	if (vdif < 0.)
	{
		q = powl(c0, 2) * ro0s[k] * (as[k] + aks[k]) / (v + vk) * powl(vdif, 2) +
			cl * speedsound * ro0s[k] * sqrtl(0.5 * (as[k] + aks[k])) * abs(vdif) /
			(0.5 * (v + vk));
	}
	...
	/*I2p[k] = 0.5*(powl(epsrrp[k], 2) + powl(epszzp[k], 2) + 2*powl(epsrzp[k], 2) + powl(epsrrp[k]+epszzp[k], 2));
		   */
	I2p[k] = 0.5*(powl(epsrrp[k]-tetp[k]/3., 2) + powl(epszzp[k]-tetp[k]/3., 2) +
		2*powl(epsrzp[k], 2)); //Версия 2015 - надо писать так!
		//(2015 год) Ололололо!!!
	sqI2p[k] = sqrtl(I2p[k]);
	...
	// ЭНЕРГИЯ
	ener[k] = 	ener[k] + rcent[k] * dtime *
				(sigmarr[k] * ((z1 - z2) * u3 + (z2 - z3) * u1 + (z3 - z1) * u2) +
				sigmazz[k] * ((r2 - r1) * v3 + (r1 - r3) * v2 + (r3 - r2) * v1) +
				sigmarz[k] * ((r2 - r1) * u3 + (r1 - r3) * u2 + (r3 - r2) * u1 +
				(z1 - z2) * v3 + (z2 - z3) * v1 + (z3 - z1) * v2) +
 // (2015 год) А мне нравится 3.e0. В этом числе есть глубокая загадочность
				sigmatt[k] * square[k] * (u1 + u2 + u3) / (3.e0 * rcent[k])) /
				amaselm[k];
	...
	Y2[k] = 	powl(srr[k], 2.) + powl(szz[k], 2.) + powl(srz[k], 2.) +
				srr[k] * szz[k];
...
void __fastcall TmainForm::RefreshClick(TObject *Sender)
{
	...
	graphForm->Canvas->FillRect(Rect(0, 0, graphForm->ClientWidth,
		graphForm->ClientHeight));
...
//		AnsiString("wt").c_str());
	...
	{
		graphForm->Canvas->Brush->Color = clWhite;
		graphForm->Canvas->FillRect(
			Rect(0, 0, graphForm->ClientWidth, graphForm->ClientHeight)
		);
...
// (2015 год) ИЛИ ЗДЕСЬ ОШИБКА???
	...
	if (CheckBoxinss->Checked)
	{
		...
		if (RadioGroup1->ItemIndex == 0)
		{
			for( int i = 1; i <= n4 + 1; i++)
				speedz[i] = 0.;
			for( 	int i = h2i[1] / (h0 / n1) + 1;
					i <= (n4) * (n2 + 1);
					i += n4 + 1
			)
				speedr[i] = inss;
			for(	int i = (n4 + 1) * (n2) + 1; 
					i <= (n4 + 1) * (n2) + h2i[1] / (h0 / n1);
					i++
			)
				// int i=(n4+1)*(n2);
				speedz[i] = inss;
		...
		if (RadioGroup1->ItemIndex == 1)
		{
			for (int i = 1; i <= n4 + 1; i++)
				speedz[i] = 0;
			for (int i = h2i[1] / (h0 / n1) + 1; i <= (n4) * (n2 + 1);
			i += n4 + 1)
				speedr[i] = 0;
			for (int i = (n4 + 1) * (n2 + 1) + 1;
			i <= (n4 + 1) * (n2 + 1) + h2i[1] / (h0 / n1); i++)
				speedz[i] = 0;
		}
	}
	...
	for (int n = 0; n <= nt; n++)
	{
		...
		L1:
			...
			for (int k = 1; k <= numberelem; k++)
			{
				...
				long double sum[3];
				sum[0] = powl(zcoord[it3] - zcoord[it1], 2) +
					powl(rcoord[it3] - rcoord[it1], 2);
				sum[1] = powl(zcoord[it2] - zcoord[it3], 2) +
					powl(rcoord[it2] - rcoord[it3], 2);
				sum[2] = powl(zcoord[it1] - zcoord[it2], 2) +
					powl(rcoord[it1] - rcoord[it2], 2);
...
// (2015 год) ТОЧНО ПРОВЕРИТЬ НА ОШИБКИ!!!!
			...
			if( CheckBoxnp3->Checked ) // Нижняя пространственная волна 3
			{
				for( int i = 0; i < n4; i++ )
				{
					...
					{
						speedz[i] = abs(vindent * sin(M_PI_2 * 8. * rcoord[i] / radius1) *
									sin(M_PI_2 - M_PI_2 * n / nt));
					}
					else
					{
						...
						{
							speedz[i] = abs(vn2 * sin(M_PI_2 * 4. * rcoord[i] / radius1) *
										sin(M_PI_2 - M_PI_2 * n / nt));
						}
						else
						{
							...
							{
								speedz[i] = abs(vindent * sin(M_PI_2 * 8. * rcoord[i] / (3. * radius1)) *
											sin(M_PI_2 - M_PI_2 * n / nt));
							}
							else
							{
								...
								{
									speedz[i] = abs(vn2 * sin(M_PI_2 * 2. * rcoord[i] / radius1) *
												sin(M_PI_2 - M_PI_2 * n / nt));
								}
							}
						}
...
// (2015 год) Проверить на ошибки!!!
					if (n < nt / 2) // <-- (2015 год) Проверить здесь
						speedr[i] = -vb * sin(n * M_PI / (nt / 2)); // <-- (2015 год) и здесь
					else
						speedz[i] = 0;
				}
			}
			...
			if (CheckBoxbsh->Checked) // Волна Индентора
			{
				...
				if (RadioGroup1->ItemIndex == 1) // Синусоидальная
				{
					...
					for(	int i = (n2 + 1) * (n4 + 1) + (n1) * (n3);
						...
						if (n < nt / 2)
						speedz[i] = -bsh * sin(n * M_PI / (nt / 2)); // <-- (2015 год) Проверить здесь
					// else speedz[i] = 0;
					for (int i = 1; i <= n4 + 1; i++)
					{
						speedr[i] = 0.;
...
//(Ales'hon'ne 15.2.5) Сатана доволен своими учениками.
	...
	if (mainForm->CheckBox4->Checked)//(Ales'hon'ne 15.2.5) горизонтальные слои
	{
		for (int i = 1; i <= nstrat; i++)
...
// (2015 год) Проверить на ошибки!!!
	...
	int tmat /*= matstrat0*/;
	int tcrdr=-1,tcrdz=-1; //magic is here - координаты текущего квадратика с двумя треугольничками
	for( int l = 1; l <= n3 * (n1 - 1); l++ )  // <-- (2015 год) Новая реализация
	{
		...
		{
			if( (l == trast) + (l == trast - 1) + (l == trast - 2) + (l > ((n3 - 2) * (n1))) ) // <-- (2015 год) Подозрительный код
			{
				if (l == trast)
					trast += n3;
				matelm[kn] = tmat;
			...
			{
				if ((l > ((n3 - 10) * (n1))) * (l < ((n3 - 8) * (n1)))) // <-- (2015 год) Подозрительный код
				{
					matelm[kn] = tmat;
					matelm[kv] = tmat;
				}
			...
			{
				if ((l > ((n3 - 14) * (n1))) * (l < ((n3 - 12) * (n1)))) // <-- (2015 год) Подозрительный код
				{
					matelm[kn] = tmat;
					matelm[kv] = tmat;
				}
				if ((l > ((n3 - 8) * (n1))) * (l < ((n3 - 6) * (n1)))) // <-- (2015 год) Подозрительный код
				{
					matelm[kn] = tmat;
					matelm[kv] = tmat;
				}
			...
			if (mainForm->ComboBoxBuild->ItemIndex == 3)
			{
				...
				if ((l > ((n3 - 6) * (n1))) * (l < ((n3 - 4) * (n1)))) // <-- (2015 год) Подозрительный код
				{
					matelm[kn] = tmat;
					matelm[kv] = tmat;
				}
...
void __fastcall TmainForm::SquareLoop(TObject *, int k)
{
		...
		square[k] = abs(0.5 * (rcoord[it2] * (zcoord[it3] - zcoord[it1]) +
			rcoord[it1] * (zcoord[it2] - zcoord[it3]) + rcoord[it3] *
			(zcoord[it1] - zcoord[it2])));
...
void _fastcall TmainForm::threegraphs(bool save, int i) {
	...
	//bmp->Free();
	if (save) {
		bmp->SaveToFile("Graph"+IntToStr(i)+".bmp");
	}
	delete bmp;
}
// *****************************************************************
...
TColor TempToColor(long double T) {
	struct MyRGB {
		...
		{0, 0, 0, 0.}, {70, 70, 70, 300.}, {175, 43, 30, 350.},
		{255, 255, 0, 400.}, {255, 255, 255, T_max}};
	if (T >= TG[4].T) {
		return TG[4].color();
	}
	if (T <= TG[0].T) {
...
void threeangle(int k)
{
	...
	if (Colorletch[k - 1] != 0)
	{
		...
		// if (I2p[k]>0.01)
		if(	Material[matelm[k]].sigma1 > Material[matelm[k]].sigma0 -
			abs(Material[matelm[k]].alpha * I2p[k]) )
		{
			if (matelm[k] != 9)
			{
				DefColor = clRed;
...
// (2015 год) Проверить на ошибки!!!
	for (int il = 0; il <= 2; il++)
	{
		poly[il] = 	Point((graphForm->ClientWidth + WidthCoef1 +
					rcoord[itop[il + 1][k]] * WidthCoef) / 2,
...
void newspeed()
{
	...
	for (int i = 1; i <= numbertop; i++)
	{
		...
		for (int j = 1; j <= nearel[i]; j++)
		{
			...
			sumr +=	rcent[k] * (sigmarr[k] * zc + sigmarz[k] * rc) +
					2. / 3. * square[k] * sigmatt[k];
...
void __fastcall TmainForm::GetBeginValue()
{
	for (int i = 0; i < nel; i++)
	{
		srr[i] = szz[i] = srz[i] = stt[i] = epsrr[i] = epszz[i] = epsrz[i] =
			epstt[i] = matelm[i] = epsrrp[i] = epszzp[i] = epsrzp[i] =
			epsttp[i] = sigmarr[i] = sigmazz[i] = sigmarz[i] = sigmatt[i] =
			ener[i] = I2p[i] = 0.;
	}
	for (int i = 0; i < nus; i++)
	{
...
void GraficRefresh(int n)
{
	...
	static int *Points[ 9 ] = {
		NULL, // (2015 год) Это не ошибка. Так нужно.
		CPointTarget, CPointImpactor, SPointTarget, SPointImpactor,
		BPointTarget, BPointImpactor, TPointTarget, TPointImpactor
	};
	static int *Elems[ 9 ] = {
		NULL, // (2015 год) Это не ошибка. Так нужно.
		CElemTarget, CElemImpactor, SElemTarget, SElemImpactor,
		BElemTarget, BElemImpactor, TElemTarget, TElemImpactor
	};
	static const int numMagic = 24;
	static long double *magic[ numMagic ] =
	{
		NULL, // (2015 год) Это не ошибка. Так нужно.
		rcoord, zcoord, speedr, speedz,
		epsrr, epszz, epsrz, epstt,
		epsrrp, epszzp, epsrzp, epsttp,
		sigmarr, sigmarz, sigmazz, sigmatt,
		sqI2p, tet, tetp, ener,
		T, T_plast, T_elast
	};
	for( int pl = 1; pl <= 8; ++pl )
	{
		...
		switch( pl )
		{
		case 1: case 3: qwer = n2; break;
		case 2: case 4: qwer = n1; break;
		case 5: case 7: qwer = n4; break;
		case 6: case 8: qwer = n3; break;
		default: throw("Unknow error!");
		}
...
void draw()
{
	...
	case 5: case 7: qwer = n4; break;
	case 6: case 8: qwer = n3; break;
	default: throw( " Ищите switch " );
	}

	graficForm->Chart1->Title->Text->Clear();
	graficForm->Chart1->Title->Text->Add(
		graficForm->ComboBox1->Text + " (" + graficForm->ComboBox2->Text + ")");
...
void __fastcall TmainForm::nstratBoxChange(TObject *) {
	int l = nstratBox->ItemIndex; bool E2( l ), E3( l&&(l-1) );
